/* Direct source adaptation of SukiSU Ultra's InnerShadow.kt (Apache-2.0). */
...
private class InnerShadowNode(
    ...
    override fun ContentDrawScope.draw() {
        drawContent()
        val inner = shadow() ?: return
        val layer = shadowLayer ?: return
        val radius = inner.radius.toPx()
        val offsetX = inner.offset.x.toPx()
        val offsetY = inner.offset.y.toPx()
        ...
        paint.color = inner.color
        layer.alpha = inner.alpha
        layer.blendMode = inner.blendMode
